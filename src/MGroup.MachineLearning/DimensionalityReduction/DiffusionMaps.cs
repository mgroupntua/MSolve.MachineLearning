...
namespace MGroup.MachineLearning.DimensionalityReduction
{
	public class DiffusionMapsAlgorithm
	{
		...
		private void ProcessData()
		{
			...
			int[,] indices = NearestNeighborSearch.KnnSearch(dataSet, dataSet, numberOfKNN).Item1;
			double[,] sortedDistances = NearestNeighborSearch.KnnSearch(dataSet, dataSet, numberOfKNN).Item2;
		...
		//private static (int[,], double[,]) KnnSearch(double[,] trainSamples, double[,] testSamples, int K)
		//{
		...
		private static double[,] reshapeDt(double[,] dt, int[,] indices, int N, int k)
		{
			...
			for (int i = 0; i < N; i++)
			{
				for (int j = 0; j < N; j++)
				{
					reshapedDt[j, i] = dt[temp2[count], temp3[count]];
					count += 1;
				}
			}
			return reshapedDt;
